...
def generate_batch_factor_code(ground_truth_data, representation_function,
  ...
  i = 0
  print()
  print("RANDOM STATE ", type(random_state))
    ...
    if i == 0:
      ...
      if _try:
        try:
          ...
          if _try:
            _try = False
            representations = representation_function(current_observations.reshape((current_observations.shape[0], -1)))
      else:
        representations = representation_function(current_observations.reshape((current_observations.shape[0], -1)))
    else:
  ...
  for i in range(num_codes):
    for j in range(num_factors):
      try:
        ...
        m[i, j] = sklearn.metrics.mutual_info_score(ys[j, :], np.squeeze(mus[i, :]))
  return m
...
def _histogram_discretize(target, num_bins=50):
...
def make_discretizer(target, num_bins=50,
                     discretizer_fn=None):#gin.REQUIRED):
  """Wrapper that creates discretizers."""
  try:
    return discretizer_fn(target, num_bins)
  except:
    return _histogram_discretize(target, num_bins)
